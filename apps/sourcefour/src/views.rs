...
struct DiffView {
    ...
    before_image: Option<Arc<gpui::Image>>,
    /// Renderable new-side image, wrapped once per content.
    after_image: Option<Arc<gpui::Image>>,
...
impl SourcefourWindow {
    ...
    fn open_branch_dialog(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) {
    ...
    fn branch_start(&self) -> Option<sourcefour_model::Oid> {
    ...
    fn submit_branch_dialog(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) {
    ...
    fn image_split_view(&self, view: &DiffView) -> Div {
        div()
            .size_full()
            .flex()
    ...
    fn image_layer(image: Option<Arc<gpui::Image>>) -> Div {
        div()
            .absolute()
            .flex()
            .items_center()
    ...
    fn branch_overlay(&self, cx: &mut gpui::Context<Self>) -> Option<impl IntoElement + use<>> {
